...
function getDistance(searchResult, searchName) {
    ...
    d3.json("data/feature-collection.json").then(data => {
        hospitals = data;
        ...
        console.log(`The closest hospital to your search location is ${name}, which is ${formattedDistance} miles away.`)
        console.log(`You searched for ${searchName}`)

        //results.innerHTML = `The closest hospital to your search location is <b>${name}</b>,<br>which is ${formattedDistance} miles away.`
        results.innerHTML = `Nearest hospital: <strong>${name}</strong> (${formattedDistance} miles away)`
    })
}
